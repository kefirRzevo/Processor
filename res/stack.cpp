...
{  
    ...
    switch (current_cmd)
    {
        case _ctor: strcpy(function, "stack_ctor");   break;
        case _dtor: strcpy(function, "stack_dtor");   break;
        case _push: strcpy(function, "stack_push");   break;
        case  _pop: strcpy(function, "stack_pop" );   break;
        case _dump: strcpy(function, "stack_dump");   break;
    }                               
    fprintf(LOGFILE, "|%-12s|%-12s|", stack_name, function);                            
    if(current_cmd == _push && ( (error & INVALID_DATA) == 0))                           
        fprintf(LOGFILE, "%-14" SPECIFICATOR "|", element);                             
    else if(current_cmd == _pop && ( (error & INVALID_EMPTY_STACK) == 0))                
...
void stack_ctor(stack *const p_stack, const char *const stack_name)
{
       ...
       p_stack->data     != nullptr) 
    {
        ...
        check_stack(p_stack, stack_name, _ctor, error, nullptr, 0);
...
final_check:
    check_stack(p_stack, stack_name, _dtor, error, nullptr, 0);
}
...
void stack_push(stack *const p_stack, const char *const stack_name, const item_t element)
{
    ...
    if(error)
    {
        check_stack(p_stack, stack_name, _push, error, nullptr, element);
...
void stack_pop(stack *const p_stack, const char *const stack_name, item_t *const p_element)
{
    ...
    if(error || p_stack->size == 0)
    {
        ...
        check_stack(p_stack, stack_name, _pop, error, p_element, 0);
